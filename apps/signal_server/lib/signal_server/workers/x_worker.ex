defmodule SignalServer.Workers.XWorker do
  use GenServer
  require Logger
    ...
    new_state =
      case fetch_signals() do
        {:ok, signals} ->
          ...
          if new != [] do
            Phoenix.PubSub.broadcast(SignalServer.PubSub, "signals", {:x_signals, signals})
          end
          %{state | last: signals}
        {:error, err} ->
          Logger.warning("XWorker: #{inspect(err)}")
          state
      end
  ...
  defp fetch_signals do
    ...
    case Req.get("#{geass_url}/api/trends/x-signals") do
