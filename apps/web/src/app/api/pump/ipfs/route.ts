...
/**
 * Fetches a user-supplied image URL server-side, defending against the most
 * obvious SSRF abuse. Only http(s) URLs, image/* responses, ≤5 MB.
 */
...
export async function POST(request: Request) {
  ...
  // Resolve image: prefer uploaded `file`, otherwise fetch `imageUrl` server-side.
  ...
  if (!(existing instanceof Blob) || existing.size === 0) {
    if (!imageUrl) {
      return NextResponse.json(
        { error: "Image required. Upload a file or provide an image URL." },
        { status: 400 },
      );
    }
    try {
  ...
  // Defensive: pump.fun rejects empty name/symbol.
  ...
  try {
    ...
    if (!upstream.ok) {
      ...
      const hint = upstream.status === 500
        ? " — pump.fun rejected the request. Common causes: image format unsupported, image too large, or upstream is rate-limiting from this region."
        : "";
      return NextResponse.json(
