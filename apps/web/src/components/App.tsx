...
import { connectPhantom, signAndSendBytes } from "@/lib/wallet";
...
export function App() {
  const [tab, setTab]         = useState<"gems" | "feed" | "launch">("gems");
  ...
  // Merge stream gems into the main list
  useEffect(() => {
  ...
  const onConnectWallet = useCallback(async () => {
    try {
      ...
      setWBal(sol === null ? null : sol.toFixed(3));
    } catch (e) {
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: "#09090b", color: "#f4f4f5", fontFamily: "'Inter',system-ui,sans-serif", overflow: "hidden" }}>
      ...
      <aside style={{ width: 200, background: "#0c0c0e", borderRight: "1px solid #18181b", display: "flex", flexDirection: "column", flexShrink: 0 }}>
          ...
          {NAV.map(n => (
            <button key={n.id} onClick={() => setTab(n.id as typeof tab)}
              style={{ width: "100%", display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", borderRadius: 8,
                border: `1px solid ${tab === n.id ? "#dc262640" : "transparent"}`,
                background: tab === n.id ? "#dc262612" : "transparent",
                color: tab === n.id ? "#ef4444" : "#52525b",
        ...
        <div style={{ padding: 8, borderTop: "1px solid #18181b" }}>
          <button onClick={onConnectWallet}
            ...
            {wallet ? `✓ ${shortAddr(wallet)}${wBal ? ` · ${wBal}◎` : ""}` : "◎ Connect Phantom"}
          </button>
        </div>
      </aside>
      ...
      <main style={{ flex: 1, overflow: "auto" }}>
        ...
        {tab === "gems" && (
          <div style={{ padding: "18px 22px" }}>
            ...
            <p style={{ fontSize: 11, color: "#3f3f46", marginBottom: 16 }}>Real-time detection · Helius pump.fun + DexScreener · Server-side scan, SSE push</p>
        ...
        {tab === "launch" && (
          <div style={{ padding: "18px 22px", maxWidth: 500 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", padding: "8px 12px", marginBottom: 14, background: "#111113", border: `1px solid ${wallet ? "#10b98130" : "#dc262630"}`, borderRadius: 8 }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 10, color: wallet ? "#10b981" : "#ef4444", fontWeight: 600 }}>
                  {wallet ? `✓ ${wallet.slice(0, 16)}...` : "Connect Phantom"}
                </div>
                {wBal && <div style={{ fontSize: 9, color: "#3f3f46" }}>{wBal} SOL</div>}
              </div>
              <button onClick={onConnectWallet} style={{ background: wallet ? "#10b98110" : "#dc262610", border: `1px solid ${wallet ? "#10b98130" : "#dc262640"}`, color: wallet ? "#10b981" : "#ef4444", padding: "5px 12px", borderRadius: 6, fontSize: 9, fontWeight: 700, cursor: "pointer" }}>{wallet ? "Connected" : "Connect"}</button>
            </div>
            {ctStep === "form" && (
